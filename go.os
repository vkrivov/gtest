...
Процедура Инициализировать()
	...
	ФиксированныеНастройки.Вставить("МаркерНазваниеИБ",			"Название:");
	ФиксированныеНастройки.Вставить("МаркерСтрокаСоединения",	"Строка соединения:");
	
	// Настройки, специфичные для определённого проекта, задаются во внешнем файле 
	...
	ГлобальныеПеременные.Вставить("АргументыКоманднойСтроки",	Новый Массив());
	ГлобальныеПеременные.Вставить("КомментарийККоммиту",		"");
...
Процедура ПодготовитьСпискиОбъектов()
	...
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		...
		Если СтрНачинаетсяС(ИсходнаяСтрока, ФиксированныеНастройки.МаркерНазваниеИБ) Тогда
			Название = Сред(ИсходнаяСтрока, СтрДлина(ФиксированныеНастройки.МаркерНазваниеИБ) + 2);
			Продолжить;
		КонецЕсли;
		...
		Если Название <> "" И СтрокаСоединения <> "" Тогда
			БазыДанных.Вставить(0, НовыйОписаниеБазы());
			БазыДанных[0].Название = Название;
...
Процедура ВыгрузитьОбъектыВФайлы()
	...
	Для каждого СвойстваБД Из БазыДанных Цикл
		...
		// При линейном способе выгрузки (-Format Plain) модули имеют расширение txt, а всё остальное - xml
		// Сканировать будем только модули, но для того, чтобы сканер из увидел, нужно сменить расширение на bsl
...
Процедура РазобратьКоманднуюСтроку()
	...
	//  все остальные аргументы склеиваются в строку, которая будет использоваться как комментарием к коммиту
	Для каждого Аргумент Из АргументыКоманднойСтроки Цикл

		Если СтрНачинаетсяС(Аргумент, "-") Тогда
			ГлобальныеПеременные.АргументыКоманднойСтроки.Добавить(Сред(Аргумент, 2));
		Иначе
	...
	Если ГлобальныеПеременные.АргументыКоманднойСтроки.Найти("nc") <> Неопределено Тогда
	...
	ГлобальныеПеременные.КомментарийККоммиту = Комментарий;
		
КонецПроцедуры

// Производятся необходимые опреации с Git 
//  поскольку в настоящее время мы работаем с локальным ропозиторием, 
...
Процедура Коммит()
	...
	СтрокаЗапуска = СтрШаблон(
		СтрокаЗапускаШаблон,
		СтрокаВКавычках(ГлобальныеПеременные.КомментарийККоммиту));
